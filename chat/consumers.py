...
class ChatConsumer(WebsocketConsumer):
    ...
    def receive(self, text_data):
        ...
        if self.validate_receiver(receiver):
            ...
            new_message = Message(sender=data["sender"], receiver=receiver, message=data["message"])
            new_message.save()
            ...
            async_to_sync(self.channel_layer.group_send)(
                ...
                {
                    "type": "chat.message",
                    "message": {"message": data["message"], "timestamp": new_message.timestamp},
                }
            )
        
    def chat_message(self, event):
        self.send(text_data=json.dumps(event["message"], indent=4, sort_keys=True, default=str))